...
public class GoOutsideChanceProvider : ActionChanceProvider
{
    ...
    {
        // 基礎成功率（百分比轉 0~1）
        float chance = (p.LifePower * lifePowerWeight + p.Sociality * socialityWeight) / 100f;
    ...
    {
        float raw = (p.LifePower * lifePowerWeight + p.Sociality * socialityWeight) / 100f;
        return $"GoOutside: raw={raw:P0}, final={chance:P0}, " +
               $"LifePower={p.LifePower}, Sociality={p.Sociality}, Stress={p.Stress}";
    }
}
